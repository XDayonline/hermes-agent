...
class GatewaySessionCommandsMixin:
    ...
    async def _list_titled_sessions(self, source, session_key: str, allow_all: bool,
        ...
        sessions = await self._session_db.list_sessions_rich(
            ...
            session_key=None if widen else session_key, limit=limit)
    ...
    async def _handle_resume_command(self, event: MessageEvent) -> str:
        ...
        if not name:
            # Telegram (and any other adapter with send_sessions_picker) gets a paginated
            # inline keyboard; every other adapter falls back to the numbered list below.
            adapter = self.adapters.get(source.platform) if source else None
            if adapter is not None and getattr(type(adapter), "send_sessions_picker", None) is not None:
                try:
                    picker_sessions = await self._list_titled_sessions(
                        source, session_key, allow_all, limit=50)
                    if picker_sessions:
                        _self = self
                        _session_key = session_key

                        async def _on_session_selected(_chat_id: str, target_id: str) -> str:
                            return await _self._resume_to_session_id(
                    ...
                    logger.debug("Session picker failed, falling back to list: %s", e)
            try:
                titled = await self._list_titled_sessions(source, session_key, allow_all)
        ...
        return await self._resume_to_session_id(
            session_key=session_key,
            source=source,
            target_id=target_id,
            name=name,
    ...
    async def _resume_to_session_id(
        self,
        *,
        session_key: str,
        source,
        ...
        """Resolve a session id, switch the active session, and return a confirmation string.

        Shared between ``_handle_resume_command`` and the Telegram session-picker callback."""
        ...
        denied = await self._resume_access_denied_reply(
            source, target_id, name, allow_all, allow_cross_room)
        ...
        # See _CONVERSATION_SCOPED_STATE in gateway/run.py.
        ...
        # session_id end-to-end — mirrors /branch and /reset. Without this, the cached AIAgent (and
        # its memory provider, which cached `_session_id` during initialize()) keeps writing into
        # the wrong session's record. See #6672.
        self._evict_cached_agent(session_key)
        ...
        try:
            ...
            # The resume itself succeeded; only the count is missing. Say the history is
            # unreadable rather than reporting an empty session (#100788).
