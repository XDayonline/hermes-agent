...
from hermes_cli.auth import (  # resolve_external_process_provider_credentials is read via origin by runtime_provider_backends
    ...
    resolve_qwen_runtime_credentials, resolve_antigravity_oauth_runtime_credentials, resolve_api_key_provider_credentials,
...
_POOL_ENTRY_SIMPLE_MODES: Dict[str, tuple] = {
    ...
    "google-antigravity": ("chat_completions", "antigravity-pa://google"),
...
def _explicit_antigravity(requested_provider, model_cfg, api_key, base_url, target_model):
    """Explicit --api-key/--base-url for Antigravity wins over the stored agy OAuth token."""
    base_url = base_url or "antigravity-pa://google"
    if api_key:
        return _runtime("google-antigravity", "chat_completions", base_url, api_key, source="explicit",
                        requested_provider=requested_provider)
    creds = resolve_antigravity_oauth_runtime_credentials()
    return _runtime("google-antigravity", "chat_completions", creds.get("base_url", "") or base_url,
                    creds.get("api_key", ""), source="explicit", expires_at_ms=creds.get("expires_at_ms"),
                    email=creds.get("email", ""), project_id=creds.get("project_id", ""),
                    requested_provider=requested_provider)
...
_EXPLICIT_RESOLVERS: Dict[str, Callable[..., Dict[str, Any]]] = {
    ...
    "google-antigravity": _explicit_antigravity,
...
def _antigravity_oauth_runtime(provider, requested_provider) -> Dict[str, Any]:
    """Runtime from the local agy/Antigravity OAuth token (Gemini/Claude where entitled)."""
    creds = resolve_antigravity_oauth_runtime_credentials()
    return _runtime(provider, "chat_completions", creds.get("base_url", "") or "antigravity-pa://google",
                    creds.get("api_key", ""), source=creds.get("source", "antigravity-oauth"),
                    expires_at_ms=creds.get("expires_at_ms"), email=creds.get("email", ""),
                    project_id=creds.get("project_id", ""), requested_provider=requested_provider)
...
def _resolve_requested_shortcuts(requested_provider, explicit_api_key, explicit_base_url, target_model) -> Optional[Dict[str, Any]]:
    ...
    # Antigravity is backed by the local agy/Antigravity OAuth token. Resolve it before named-custom
    # providers and credential pools so a stale manual entry cannot hijack runtime routing.
    if requested_provider in ("google-antigravity", "antigravity", "antigravity-oauth", "antigravity-cli",
                              "agy", "agy-cli"):
        return _antigravity_oauth_runtime("google-antigravity", requested_provider)
    return None
...
def resolve_runtime_provider(*, requested: Optional[str] = None, explicit_api_key: Optional[str] = None,
    ...
    rung returns or raises, else falls to the next):
      ...
      2. requested-name shortcuts: moa, anthropic@azure, azure-foundry, vertex, antigravity
      ...
      7. OAuth specs (nous/codex/xai/qwen; "auto" swallows AuthError and logs) → antigravity
         → minimax-oauth
...
def _ladder_rungs(requested_provider, explicit_api_key, explicit_base_url, target_model):
    ...
    if provider == "google-antigravity":
        try:
            yield _antigravity_oauth_runtime(provider, requested_provider)
        except AuthError:
            if requested_provider != "auto":
                raise
